// shopcart-payment.js - MASTER CART & PAYMENT LOGIC V1.2
// Zero-Omission Protocol: 100% Original Logic Preserved + UI Null-Check Fix
...
async function submitFinalOrder() {
    ...
    try {
        ...
        localStorage.setItem('pizz_user_phone', userPhone);
        
        // THE FIX: Safe DOM Checking
        const khNameEl = document.getElementById('success-name-kh');
        if(khNameEl) khNameEl.innerText = userName;
        
        const enNameEl = document.getElementById('success-name-en');
        if(enNameEl) enNameEl.innerText = userName;
