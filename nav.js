// nav.js - MASTER BOTTOM NAVIGATION ENGINE V2.8
// Zero-Omission Protocol: 5-Slot Architecture (Orders Swapped, Sale Removed)
...
function loadBottomNav(activePage) {
    ...
    const getActiveBox = (page) => activePage === page ? 
        `background: ${activeBg}; border-radius: 8px; padding: 6px 8px; color: ${activeColor}; font-weight: 700;` : 
        `color: ${inactiveColor}; font-weight: 400; padding: 6px 8px;`;

    const navHTML = `
    <nav style="position: fixed; bottom: 0; left: 50%; transform: translateX(-50%); width: 100%; max-width: 480px; height: 65px; background: #ffffff; border-top: 1px solid #f2f2f2; display: flex; justify-content: space-around; align-items: center; z-index: 2500; box-shadow: 0 -5px 20px rgba(0,0,0,0.05); padding-bottom: env(safe-area-inset-bottom); border-radius: 15px 15px 0 0;">
        
        <a href="index.html" style="display: flex; flex-direction: column; align-items: center; text-decoration: none; font-size: 0.75rem; transition: 0.2s; width: 20%; ${getActiveBox('home')}">
        ...
        <div style="position: relative; top: -16px; text-align: center; width: 20%;">
            <a href="javascript:void(0)" onclick="if(window.AIChatEngine) { AIChatEngine.toggleChat(); } else { alert('AI is loading...'); }" style="display: flex; flex-direction: column; align-items: center; text-decoration: none; transition: 0.2s;">
                <div style="width: 54px; height: 54px; background: #e74c3c; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 12px rgba(231, 76, 60, 0.4); border: 3px solid #ffffff; margin-bottom: 4px; margin-left: auto; margin-right: auto;">
                ...
                <span style="font-size: 0.75rem; font-weight: 800; color: #e74c3c;">ប្រើខ្ញុំ</span>
            </a>
        </div>
